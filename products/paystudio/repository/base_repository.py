...
class BaseRepository(ABC):
    """
    Clase base abstracta para todos los repositories de PayStudio.
    Define el contrato común de persistencia y lógica de infraestructura compartida.
    """

    @staticmethod
    def log_sql(modulo: str, operacion: str, query: str, params=None, resultado=None):
        """Traza una operación SQL en el log a nivel DEBUG."""
    ...
    def esperar_no_existencia(
        ...
        """Espera activa hasta que un registro deje de existir en DB."""
        logger.info(f"INICIO: Esperando eliminación física de '{nombre}' en la base de datos...")

        inicio = time.time()
        while time.time() - inicio < timeout_segundos:
            logger.debug(
                f"Reintentando verificación para '{nombre}'... "
                f"(Tiempo: {round(time.time() - inicio, 1)}s)"
            )
